...
# A workaround in https://github.com/JuliaNonconvex/NonconvexSemidefinite.jl/issues/6
using AbstractDifferentiation, ForwardDiff, ChainRulesCore

function ChainRulesCore.rrule(::typeof(expected_xn), x⁴, x², n)
   val, ds = AD.value_and_derivative(AD.ForwardDiffBackend(), expected_xn, x⁴, x², n)
   return val, Δ -> (NoTangent(), (Δ .* ds)...)
end
