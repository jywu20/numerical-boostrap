...
我们采用Arpack-NG库（用于大型矩阵对角化的Arpack库的一个实现）\cite{arpackcode}的Julia封装\texttt{Arpack.jl}\cite{arpackjl}完成这项工作。取$L=10$以及不同的$\Delta x$，使用\texttt{Arpack.jl}计算$-\laplacian + x^2$的最低本征态，即系统基态，所得结果见\prettyref{tbl:stationary-benchmark}。
