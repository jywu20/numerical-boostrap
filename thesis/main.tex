...
\begin{abstract}
    ...
    本文回顾了数值bootstrap方法的形式理论和已经报道的应用，介绍了它相比已有的非微扰数值方法的优势，复现了\parencite{han_matrix}中非简谐振子的非线性SDP数值bootstrap并自行完成了其线性SDP，并与定态薛定谔方程比较，验证了其正确性，并发现了高精度计算时的收敛性问题；复现了\parencite{han_manybody}中一维和二维Hubbard模型的线性SDP数值bootstrap，观察了其精度和Hubbard $U$参数和电子填充数目的关系，并定性分析了其原因；展望了数值bootstrap方法未来可能的发展和在其它模型上的广阔前景。
\end{abstract}
...
\begin{abstract*}
    ...
    non-perturbative approaches. The nonlinear SDP numerical bootstrap of the anharmonic oscillator in 
...
与前述的常规数值方法不同，bootstrap技术不处理波函数、算符、（离散）路径积分这些用于计算物理量期望值但不直接对应于实验测量值的对象，而是通过在待求解的物理量期望值之间建立解析的等式和不等式关系，据此它们的取值范围。
...
\input{hubbard-1d-doped-energy-double-occ-table.tex}
...
\input{hubbard-2d-energy-double-occ-table.tex}
...
这提示我们，为了获得基态能量，实际上并不需要全部的等式约束。如果有方法能够判断出在一个问题中，给基态能量下界设下限制的约束条件主要是什么，那么计算效率可以大大提高，因为本文考虑的Hubbard模型的线性SDP的性能瓶颈似乎主要在优化问题\emph{之前}（\prettyref{sec:hubbard-1d-result}）。
未来可以在不同的模型中撤去一些约束，观察剩下的约束的数值bootstrap的精度是否有显著下降，从而获知哪些约束对数值bootstrap是重要的。
