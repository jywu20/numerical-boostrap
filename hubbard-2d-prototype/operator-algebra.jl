...
translational_x_constraint_coefficients = map(hubbard_opstr_basis[2:end]) do op
    translated_op = hubbard_opstr_translate(op, [1, 0])
...
filter!(translational_x_constraint_coefficients) do c
    c !== nothing
end

translational_y_constraint_coefficients = map(hubbard_opstr_basis[2:end]) do op
    translated_op = hubbard_opstr_translate(op, [0, 1])
    if translated_op === nothing
        return
    end
    cons_ops = op - translated_op
...
filter!(translational_y_constraint_coefficients) do c
    c !== nothing
end
