...
function xpopstr_const(c)
    result = OffsetArray(zeros(Complex{F}, xpopspace_dim + 1), xpopspace_index_range)
    result[0] = c
    result
end
...
xpopstr_expected = Vector{Union{F, VariableRef}}(undef, 2xpopspace_dim)
for opstr_index in 1 : xpopspace_dim
    ...
    if isodd(x_power + p_power)
        xpopstr_expected[2opstr_index - 1] = F(0.0)
        xpopstr_expected[2opstr_index] = F(0.0)
        continue
    end
    ...
    # iseven(p_power)
    # Real part only 
    xpopstr_expected[2opstr_index - 1] = @variable(model, base_name = "re(x^$x_power p^$p_power)")
    xpopstr_expected[2opstr_index] = F(0.0)
end
...
if check_feasibility
    ...
    feasibility_report = primal_feasibility_report(model, 
        Dict{VariableRef, F}(filter(benchmark_point) do p
